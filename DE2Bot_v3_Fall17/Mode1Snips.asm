...
;Top: resets positions and enables the static sonars, then loops for 10 seconds
Top:
	LOAD 	StaticMask
	OUT 	SONAREN
	OUT		RESETPOS
	CALL 	Wait1		; sonar must initialize
	OUT 	TIMER
	CALL	Cached0145
...
TopLoop:
	...
	CALL 	Abs
	SUB 	SonarError
	JPOS 	TopDetected	; check if DIST0 changed
	...
	CALL 	Abs
	SUB 	SonarError
	JPOS 	TopDetected	; check if DIST1 changed
; 	
; 	IN		DIST4
; 	SUB 	Cached4
; 	CALL 	Abs
; 	SUB 	SonarError
; 	JPOS 	TopDetected	; check if DIST4 changed
; 	
; 	IN 		DIST5
; 	SUB 	Cached5
; 	CALL 	Abs
; 	SUB 	SonarError
; 	JPOS 	TopDetected	; check if DIST5 changed
; 	
; 	IN 		DIST2
; 	SUB 	StaticFront
; 	JNEG 	TopDetected	; check 2 distance < 60"
; 	
; 	IN 		DIST3
; 	SUB 	StaticFront
; 	JNEG 	TopDetected	; check 3 distance < 60"
	
	IN 		TIMER
	ADDI 	-100
	JNEG 	TopLoop		; stay for 10s
	JUMP 	GoToBottom

TopDetected:
	LOAD 	SECBEEP
	OUT		BEEP
	CALL 	Wait1			; pause while we beep

;Sets up the bot to move tot he bottom area	
GoToBottom:
	;TESTING STUFF
	LOADI 	&H0160
	OUT 	BEEP
	...
	LOAD MovingMask		; sensors 2 & 3
	OUT SONAREN
	OUT RESETPOS
	LOADI -90 			; 90 degrees to right 95 (3 degrees extra for error
	STORE DTheta
	CALL Wait1			; give it time to turn
	
	LOADI FSlow			; slowly move forward, no wheel wobble
	STORE DVel
	LOADI 0
	STORE DTheta		; magic bug fix
	OUT RESETPOS		; reset odometer
	IN DIST2
	STORE Cached2
	IN DIST3
	STORE Cached3
...
;Update the saved values of sonars 0, 1, 4, and 5.
Cached0145:
	IN 		DIST0
	STORE 	Cached0
	IN 		DIST1
...
StaticMask:		DW &B00111111		;Mask for sonars to enable when staying still
MovingMask:		DW &B00001100		;Masks for sonars to enable when moving
	
;Saved previous sonar readings
Cached0:	DW 0
Cached1:	DW 0
Cached2:	DW 0
Cached3:	DW 0
...
SonarError:	DW	100	; 50mm
StaticFront:	DW 2083	; 60"
BaffleLength:	DW 2134 ; 7 ft
SECBEEP:	DW &H0840
